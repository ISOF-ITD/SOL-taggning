...
describe Sol do
  ...
  describe '#process' do
    ...
    it "handles the case of two härad", focus: true do
      ...
      expect(out.to_s).to eq '<head><placeName>Kinnekulle</placeName></head> <p><locale>berg</locale>, <location><district type="härad">Kinne</district><district type="härad">Kinnefjärdings</district><region type="landskap">Västergötland</region></location></p>'
    end

    it "handles the case of two socknar" do
      out = sol.process('Kivik tätort, Södra Mellby och Vitaby snr, Albo hd, Skåne')
      expect(out.to_s).to eq '<head><placeName>Kivik</placeName></head> <p><locale>tätort</locale>, <location><district type="socken">Södra Mellby</district><district type="socken">Vitaby</district><district type="härad">Albo hd</district><region type="landskap">Skåne</region></location></p>'
    end

    it "for real!" do
      out = sol.process('Klagshamn samhälle, Västra Klagstorps och Tygelsjö snr, Oxie hd, Skåne')
      expect(out.to_s).to eq '<head><placeName>Klagshamn</placeName></head> <p><locale>samhälle</locale>, <location><district type="socken">Västra Klagstorps</district><district type="socken">Tygelsjö</district><district type="härad">Oxie hd</district><region type="landskap">Skåne</region></location></p>'
    end
