...
describe Element do
  ...
  describe '#isplacename?' do
    it "returns true if first word is bold and capitalised" do
      doc = Document.new "<w:document xmlns:w=''><w:r><w:rPr><w:b /></w:rPr><w:t>Locketorp</w:t></w:r></w:document>"
      element = doc.root
...
describe Solig do
  ...
  describe '#unword' do
    ...
    it "takes an optional second argument" do # FIXME Stop this madness that cascades down, and replace with atribute
      albano = loadparagraph '474-albano'
      actual = solig.unword(albano, false).to_s
      expected = "<div xml:id='Albano' type='?'><p><span type='fet'>Albano</span> område på Norra Djurgården, Stockholms stad → <span type='kursiv'>Frescati</span>.</p></div>"
      # byebug
      expect(actual).to eq expected
    end
  end
  ...
  describe 'Intensive tests for #unword' do
    ...
    it "removes the type attribute for non-initial paragraphs" do
      allal = loadparagraph '467--469-al'
      al2 = allal.elements[2]
    ...
    it "doesn’t create an empty span element on Angered"
  ...
  describe '#analyse_kursiv' do
    ...
    it "doesn’t crash on Dals Rostock" do
      dals_rostock = loaddiv '755-dals-rostock'
